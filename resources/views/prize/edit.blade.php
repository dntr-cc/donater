...
@section('content')
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content rounded-4 shadow">
                            <div class="modal-header p-4 pb-2 border-bottom-0 justify-content-center">
                                <h2 class="title fs-5" id="updatePrizeModalLabel">Редагувати приз "{{ $prize->getName() }}"</h2>
                            </div>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <span class="position-relative">
                                            <div class="card border-0 rounded-4 shadow-lg">
                                                <img id="avatarImage" src="{{ url($prize->getAvatar()) }}"
                                    ...
                                    <div class="col-md-6">
                                        <div class="row mt-3">
                                            ...
                                            @foreach(RaffleUser::TYPES as $type => $name)
                                                <div class="form-check m-2">
                                                    <input class="form-check-input" type="radio" name="raffle_type"
                                                       @if($prize->getRaffleType() === $type)
                                                           checked
                                                       @endif
                                                       id="{{ $type }}">
                                        ...
                                        <div class="row mt-3">
                                            @foreach(Prize::AVAILABLE_TYPES as $type => $name)
                                                <div class="form-check m-2">
                                                    <input class="form-check-input" type="radio" name="available_type"
                                                           @if($prize->getAvailableType() === $type)
                                                               checked
                                                           @endif
                                                           id="{{ $type }}">
                                                    <label class="form-check-label" for="{{ $type }}">
                                ...
                                <div class="row">
                                    <div class="col-md-12 mt-5 mb-4">
                                        <div class="footer-modal d-flex justify-content-between">
                                            ...
                                            <button id="updatePrize" type="submit" class="btn btn-primary me-4"
    ...
    <script type="module">
        ...
        const text = `{!! $prize->getDescription() !!}`;
        ...
        $('#updatePrize').on('click', function (e) {
            e.preventDefault();
            $.ajax({
                url: '{{ route('prize.edit', compact('prize')) }}',
                type: "PATCH",
                data: {
                    name: $('#name').val(),
                    description: window.tinymce.get('description').getContent() || '<p></p>',
                    avatar: $('#avatar').val(),
                    user_id: {{ $prize->getUserId() }},
